...
async function callClaude(prompt) {
  ...
  if (isFile || isLocalhost) {
    ...
    // Si estás en GitHub Pages (u otro host estático) y NO tienes backend, 
    // tendrías que llamar directamente a Anthropic, PERO eso expondría tu API key
    // y Anthropic bloquea CORS desde navegadores como medida de seguridad.
    // Una opción rápida para pruebas es usar un proxy CORS gratuito
    apiUrl = 'https://cors-anywhere.herokuapp.com/https://api.anthropic.com/v1/messages';
    
    // ATENCION: El servidor local 'server.js' guarda la API Key de forma segura, 
    // pero si llamas a Anthropic directo desde GitHub Pages sin backend, no tienes cómo pasar el API Key
    // sin que el usuario la introduzca o la expongas en tu código frontend.
  }

  // Si estamos llamando directo a la API de Anthropic (via proxy), los headers cambian
  const isDirectApi = apiUrl.includes('api.anthropic.com');
  const headers = isDirectApi 
    ? { 
        'Content-Type': 'application/json',
        'x-api-key': 'LA_API_KEY_DEBERIA_IR_AQUI', // ¡PELIGRO de seguridad en frontend!
